...
void CS(uint3 DTid : SV_DispatchThreadID)
{
	...
	float s1 = tWaterHeightNorth[int2(DTid.x, 0)];
	...
	float w2 = tWaterHeightEast[int2(0, DTid.x)];
	...
	SetFlux(int2(DTid.x, 0), GetFlux(tFluxSouth, coord_north_edge));
	SetFlux(int2(0, DTid.x), GetFlux(tFluxWest, coord_east_edge));
}
